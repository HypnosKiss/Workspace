...
//        var_dump(get_included_files()); //此数组中的文件表示进程启动前就加载了，所以无法reload
    ...
    public function onMessage($server, $frame){
        ...
        switch($data){
            ...
            default:
                ...
                $this->sendMessageToAllUsers($data);
                break;
        }
    ...
    public function onRequest($request, $response){
        if(stripos($request->server['request_uri'], 'favicon.ico') === false){
            ...
            $this->debug('Push messages to all users.');
            $this->sendMessageToAllUsers($request->get['message']);
        }
    }
    ...
    public function onReceive($server, int $fd, int $reactorId, string $data){
        $this->debug("Dispatch AsyncTask. data:", $data);
        //得到包体长度
        $len  = unpack(PACKAGE_LENGTH_TYPE, $data, 0)[1];
        $body = substr($data, -$len);//去除二进制数据之后,不要包头的数据
        //投递异步任务
        ['type' => $type, 'data' => $oriData] = json_decode($body, true);
        $oriData = $oriData ?? $body;
        switch($type){
            case 'reload':
                $this->debug('server reload');
                $server->reload();
                break;
            case 'task':
                $task_id = $server->task($oriData);
                $this->debug("AsyncTaskID:$task_id", 'data:', $oriData);
                #向客户端发送数据
                $server->send($fd, "received data .");
                break;
            default:
                $this->debug('Push messages to all users.');
                $this->sendMessageToAllUsers($oriData);
                break;
        }
    }
    ...
    /**
     * 发送消息给所有用户(websocket连接用户)
     * @param $data
     */
    public function sendMessageToAllUsers($data){
        foreach($this->websocket_server->connections as $current_fd){
            // 需要先判断是否是正确的websocket连接，否则有可能会push失败
            if($this->websocket_server->isEstablished($current_fd)){
                $this->websocket_server->push($current_fd, $data);
            }
        }
    }
}
...
$config = [
    ...
    // https://wiki.swoole.com/#/server/port?id=多端口监听
    ...
    // 主服务器为 HTTP/WebSocket 服务器，监听端口调用 set 设置配置参数，会清除主服务器的协议设定。监听端口将变为 TCP 协议。监听的端口如果希望仍然使用 HTTP/WebSocket 协议，
    // 需要在配置中增加 open_http_protocol => true 和 open_websocket_protocol => true
