...
namespace Crontab\CronExprParser;

use Crontab\ArgValidator\ArgValidator;
use Crontab\CronExprParser\Exception;
...
class Parser {
	...
	public static function matchTime($time, $expr){
		ArgValidator::assert($time, ['string', 'numeric']);
		ArgValidator::assert($expr, 'string');

		$cronExpr = preg_split('/\s+/', $expr, null, PREG_SPLIT_NO_EMPTY);
		if(count($cronExpr) !== 5){
			throw new Exception\InvalidArgumentException(sprintf('cron expression should have exactly 5 arguments, "%s" given', $expr));
		}
		...
		return self::matchTimeComponent($cronExpr[0], $date['minutes']) && self::matchTimeComponent($cronExpr[1], $date['hours']) && self::matchTimeComponent($cronExpr[2], $date['mday']) && self::matchTimeComponent($cronExpr[3], $date['mon']) && self::matchTimeComponent($cronExpr[4], $date['wday']);
	}
	...
	public static function matchTimeComponent($expr, $num){
		...
		if(strpos($expr, '/') !== false){
			$arg = explode('/', $expr);
			if(count($arg) !== 2){
				throw new Exception\InvalidArgumentException(sprintf('invalid cron expression component: '.'expecting match/modulus, "%s" given', $expr));
			}
			if(!is_numeric($arg[1])){
				throw new Exception\InvalidArgumentException(sprintf('invalid cron expression component: '.'expecting numeric modulus, "%s" given', $expr));
			}
		...
		elseif(strpos($expr, '-') !== false){
			$arg = explode('-', $expr);
			if(count($arg) !== 2){
				throw new Exception\InvalidArgumentException(sprintf('invalid cron expression component: '.'expecting from-to structure, "%s" given', $expr));
			}
		...
		if($from === false || $to === false){
			throw new Exception\InvalidArgumentException(sprintf('invalid cron expression component: '.'expecting numeric or valid string, "%s" given', $expr));
		}
	...
	public static function exprToNumeric($value){
		ArgValidator::assert($value, ['string', 'numeric']);
