...
class CronExpression {
	...
	public function setExpression($value){
		...
		if(count($this->cronParts) < 5){
			throw new Exception($value.' is not a valid CRON expression');
		}
	...
	public function setPart($position, $value){
		if(!$this->fieldFactory->getField($position)->validate($value)){
			throw new Exception('Invalid CRON field value '.$value.' as position '.$position);
		}
	...
	protected function getRunDate($currentTime = null, $nth = 0, $invert = false, $allowCurrentDate = false){
		$currentDate = $currentTime instanceof DateTime ? $currentTime : new DateTime($currentTime ? $currentTime : 'now');
	...
	public function isSatisfiedBy(DateTime $date, $value){
		...
		if(strpos($value, '#')){
			...
			if($weekday < 1 || $weekday > 5){
				throw new Exception("Weekday must be a value between 1 and 5. {$weekday} given");
			}
			if($nth > 5){
				throw new Exception('There are never more than 5 of a given weekday in a month');
			}
